...
public class Tools {
	...
	static {
		...
		if (RAW_HTML_ENTITY_TABLE.hashCode() != RAW_HTML_ENTITY_TABLE_HASHCODE) {
			throw new RuntimeException(
					"(INTERNAL) Malformed HtmlManipulator.RAW_HTML_ENTITY_TABLE.");
		}
	...
	public static String replaceHtmlEntities(final String s) {
		...
		for (int i = 0, n = s.length(); i < n; i++) {
			...
			if (c == '&') {
				...
				if (j >= 0) {
					final Character unicode = HTML_ENTITY_TO_UNICODE_MAP.get(s
							.substring(i + 1, j));
	...
	/**
	 * Return the date in form YYYY-MM-DD from the <b>url</b>
	 * 
	 * @param url
	 *            either already contains only YYYY-MM-DD or an URL to
	 *            sverigesradio.se website, no check if this actually is the
	 *            case
	...
	public static String getDateFromURL(String url)
		...
		if (url.contains("http://")) {
			if (url.indexOf("&date=") == -1)
				throw new InputMismatchException("Invalid input: " + url);
			int index = url.indexOf("&date=") + 6;
	...
	/**
	 * Getting a human-readable representation of the track
	 * 
	 * @param track
	 * @return String 'Artist1[, Artist2] - Name (Album)'
	 */
	public static String prntTrack(Track track) {
		if (track == null) {
			return "null";
		}
		StringBuilder sb = new StringBuilder(100);
		for (int i = 0; i < track.getArtists().size(); i++) {
			sb.append(track.getArtists().get(i).getName());
			if (i < 0 && i <= track.getArtists().size()) {
				sb.append(", ");
			}
		}
		sb.append(" - ");
	...
	/**
	 * Has it synchronized in order to only allow one action at a time. This is
	 * NOT good for the big server-farms but will do for my small project. Not
	 * my proudest solution =)
	 * 
	 * This will solve the problem that occurs if the user is faster to get the
	 * token-id from Spotify than the server is to fetch all the songs and store
	 * that in the file the user's page of error message or succeed message will
	 * hold until the writing of the file is finished.
	 * 
	 * @param write
	 *            if the operation is a writing or reading.
	 * @param filename
	 *            to read/write to
	 * @param songOnSpotify
	 *            songs to write, can be null if write is false.
	 * @return
	 */
	public static synchronized List<String> IOtoDisk(boolean write,
			String filename, TreeMap<Song, Track> songOnSpotify) {
		if (write) {
			File file = new File(Settings.cachePath, filename);
			try (OutputStream out = new DataOutputStream(new FileOutputStream(
					file))) {
				logger.fine("Saving file to " + file.getAbsolutePath());
				for (Song song : songOnSpotify.keySet()) {
					out.write(song.toString().getBytes());
					out.write(Settings.delimiter.getBytes());
					Track track = songOnSpotify.get(song);
					if (track != null)
						out.write(track.getUri().getBytes());
					else
						out.write("null".getBytes());
					// TODO change to system standard in case of windows
					out.write("\n".getBytes());
				}
			} catch (IOException e) {
				e.printStackTrace();
			...
			File file = new File(Settings.cachePath, filename);
			if (!file.exists()) {
				// the cache hasn't saved for some reason...
				return null;
			}
			try (BufferedReader br = new BufferedReader(new FileReader(file))) {
				logger.fine("read file " + Settings.cachePath + "/" + filename);
